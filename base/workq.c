...
static void BSP_FAST_TEXT_SECTION
cancel_helper_work(struct work_struct *work) {
...
static void BSP_FAST_TEXT_SECTION
destory_helper_work(struct work_struct *work) {
    struct helper_work_struct *helper = RTEMS_CONTAINER_OF(work, 
        struct helper_work_struct, work);
...
static int BSP_FAST_TEXT_SECTION
request_event_helper(struct workqueue *wq, uint32_t events, 
...
static void BSP_FAST_TEXT_SECTION 
schedule_worker(rtems_task_argument arg) {
...
static void BSP_FAST_TEXT_SECTION
schedule_delayed_timer_cb(struct timer *timer) {
	struct delayed_work *work = RTEMS_CONTAINER_OF(timer, 
		struct delayed_work, timer);
	schedule_work_to_queue(work->wq, &work->work);
}

int BSP_FAST_TEXT_SECTION 
schedule_work_to_queue(struct workqueue *wq, struct work_struct *work) {
    rtems_interrupt_lock_context lock_context;
    _Assert(wq != NULL);
    _Assert(work != NULL);
    rtems_interrupt_lock_acquire(&wq->lock, &lock_context);
...
int BSP_FAST_TEXT_SECTION
cancel_queue_work(struct workqueue *wq, struct work_struct *work, 
    bool wait) {
    _Assert(wq != NULL);
    _Assert(work != NULL);
    rtems_interrupt_lock_context lock_context;
...
int BSP_FAST_TEXT_SECTION
schedule_delayed_work_to_queue(struct workqueue *wq, 
...
int BSP_FAST_TEXT_SECTION
cancel_queue_delayed_work(struct workqueue *wq, 
...
void BSP_FAST_TEXT_SECTION
delayed_work_init(struct delayed_work *work,
    void (*handler)(struct work_struct *)) {
