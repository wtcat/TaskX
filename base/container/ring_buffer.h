...
#ifndef BASEWORK_RING_BUFFER_H_
#define BASEWORK_RING_BUFFER_H_
 ...
 */
static inline void rte_ringbuf_internal_reset(struct ring_buf *buf,
                                              int32_t value) {
 ...
 */
#define RTE_RINGBUF_DECLARE(name, size8)                                          \
    STATIC_ASSERT(size8 < RING_BUFFER_MAX_SIZE, RING_BUFFER_SIZE_ASSERT_MSG);   \
 ...
 */
#define RTE_RINGBUF_ITEM_DECLARE(name, size32)                                    \
    STATIC_ASSERT((size32) < RING_BUFFER_MAX_SIZE / 4,                          \
                 RING_BUFFER_SIZE_ASSERT_MSG);                                 \
    static uint32_t __noinit _ring_buffer_data_##name[size32];                 \
    struct ring_buf name = {.buffer = (uint8_t *)_ring_buffer_data_##name,     \
                            .size = 4 * (size32)}

/**
 * @brief Define and initialize an "item based" ring buffer.
 *
 * This exists for backward compatibility reasons. @ref RTE_RINGBUF_ITEM_DECLARE
 * should be used instead.
 *
 * @param name Name of the ring buffer.
 * @param size32 Size of ring buffer (in 32-bit words).
 */
#define RTE_RINGBUF_ITEM_DECLARE_SIZE(name, size32)                               \
    RTE_RINGBUF_ITEM_DECLARE(name, size32)
 ...
 * reasons. @ref RTE_RINGBUF_ITEM_DECLARE should be used instead.
 *
 * @param name Name of the ring buffer.
 * @param pow Ring buffer size exponent.
 */
#define RTE_RINGBUF_ITEM_DECLARE_POW2(name, pow)                                  \
    RTE_RINGBUF_ITEM_DECLARE(name, BIT(pow))
 ...
 */
#define RTE_RINGBUF_ITEM_SIZEOF(expr) DIV_ROUND_UP(sizeof(expr), sizeof(uint32_t))

/**
 * @brief Initialize a ring buffer for byte data.
 *
 * This routine initializes a ring buffer, prior to its first use. It is only
 * used for ring buffers not defined using RTE_RINGBUF_DECLARE.
 *
 * @param buf Address of ring buffer.
 * @param size Ring buffer size (in bytes).
 * @param data Ring buffer data area (uint8_t data[size]).
 */
static inline void rte_ringbuf_init(struct ring_buf *buf, uint32_t size,
                                    uint8_t *data) {
    assert(size < RING_BUFFER_MAX_SIZE);
    buf->size = size;
    buf->buffer = data;
    rte_ringbuf_internal_reset(buf, 0);
}
 ...
 *
 * This routine initializes a ring buffer, prior to its first use. It is only
 * used for ring buffers not defined using RTE_RINGBUF_ITEM_DECLARE.
 *
 ...
 */
static inline void rte_ringbuf_item_init(struct ring_buf *buf, uint32_t size,
                                         uint32_t *data) {
    assert(size < RING_BUFFER_MAX_SIZE / 4);
    rte_ringbuf_init(buf, 4 * size, (uint8_t *)data);
}
 ...
 */
static inline bool rte_ringbuf_is_empty(struct ring_buf *buf) {
 ...
 * @param buf Address of ring buffer.
 */
static inline void rte_ringbuf_reset(struct ring_buf *buf) {
    rte_ringbuf_internal_reset(buf, 0);
}
 ...
 */
static inline uint32_t rte_ringbuf_space_get(struct ring_buf *buf) {
 ...
 */
static inline uint32_t rte_ringbuf_item_space_get(struct ring_buf *buf) {
    return rte_ringbuf_space_get(buf) / 4;
}
 ...
 */
static inline uint32_t rte_ringbuf_capacity_get(struct ring_buf *buf) {
    return buf->size;
}
 ...
 */
static inline uint32_t rte_ringbuf_size_get(struct ring_buf *buf) {
 ...
 * number of bytes written must be confirmed (see @ref rte_ringbuf_put_finish).
 *
 * @warning
 ...
 * (calls prefixed with rte_ringbuf_item_).
 *
 ...
 */
uint32_t rte_ringbuf_put_claim(struct ring_buf *buf, uint8_t **data,
                               uint32_t size);
 ...
 * to all preceding @ref rte_ringbuf_put_claim invocations (or even 0). Surplus
 ...
 */
uint32_t rte_ringbuf_put(struct ring_buf *buf, const uint8_t *data,
                         uint32_t size);
 ...
 * using @ref rte_ringbuf_get_finish.
 *
 * @warning
 ...
 */
uint32_t rte_ringbuf_get_claim(struct ring_buf *buf, uint8_t **data,
                               uint32_t size);
 ...
 * all preceding @ref rte_ringbuf_get_claim invocations (or even 0). Surplus
 ...
 * multiple times. To remove data, use either @ref rte_ringbuf_get or
 * @ref rte_ringbuf_get_claim followed by @ref rte_ringbuf_get_finish with a
 * non-zero `size`.
 *
 * @param buf  Address of ring buffer.
