...
#define RTE_CACHE_LINE_SIZE CPU_CACHE_LINE_BYTES
#define RTE_CACHE_LINE_MASK (RTE_CACHE_LINE_SIZE - 1)
#define rte_pause()
#define rte_errno errno
#define rte_zmalloc(type, size, align) general_calloc(1, size)
#define rte_free(ptr) general_free(ptr)
#define rte_atomic32_cmpset(dst, exp, src) \
	_Atomic_Exchange_uint((Atomic_Uint *)dst, exp, src)
#endif
...
#include "base/log.h"
#include "base/ring/rte_ring.h"
#include "base/ring/rte_ring_elem.h"
#include "base/ring/rte_rwlock.h"


#define RTE_LOG(l, t, ...)	pr_info(# t ": ", ##__VA_ARGS__)
...
static rte_rwlock_t rte_ring_rwlock;
static struct rte_tailq_elem rte_ring_tailq = {
	.head.tailq_head = TAILQ_HEAD_INITIALIZER(rte_ring_tailq.head.tailq_head),
...
{
	rte_rwlock_read_lock(&rte_ring_rwlock);
}

static inline void rte_mcfg_tailq_read_unlock(void)
{
	rte_rwlock_read_unlock(&rte_ring_rwlock);
}

static inline void rte_mcfg_tailq_write_lock(void)
{
	rte_rwlock_write_lock(&rte_ring_rwlock);
}

static inline void rte_mcfg_tailq_write_unlock(void)
{
	rte_rwlock_write_unlock(&rte_ring_rwlock);
}

static void *
rte_ring_alloc_mem(size_t len)
{
	...
	return general_malloc(requested_len);
}
		...
		unsigned int flags)
{
	 ...
	 */
	r = rte_ring_alloc_mem(ring_size);
	if (r != NULL) {
...
rte_ring_create(const char *name, unsigned int count,
		unsigned int flags)
{
	return rte_ring_create_elem(name, sizeof(void *), count,
		flags);
}
