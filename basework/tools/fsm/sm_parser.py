...
#define _use_{name}_action_ {onoff}
'''
...
#if _use_{name}_action_
    uint32_t       action;
    union {{
        void      *param;
        uintptr_t  uparam;
    }};
#endif /* _use_{name}_action_ == 1 */
}};

#define to_derive(ctx) \\
    rte_container_of(ctx, struct {name}_context, ctx)
...
static void {method}(struct fsm_context *ctx) {{
    //struct {name}_context *p = to_derive(ctx);
    //TODO: implement
...
#if _use_{name}_action_
int {name}_fsm_execute(uint32_t action, uintptr_t param) {{
#else  /* _use_{name}_action_ == 0*/
int {name}_fsm_execute(void) {{
#endif /* _use_{name}_action_ == 1*/
    struct {name}_context *p = &{name}_ctx;
    int err;
...
def sm_parser(file):
    with open(file, 'r') as f:
        ...
        # Parse actions
        sm_action       = sm.get('action')
        sm_action_onoff = 0
        if sm_action is not None:
            action_len = len(sm_action)
            if action_len > 0:
                sm_action_onoff = 1

            for action in sm_action:
                action_name = sm_name.upper() + '_ACTION_' + action['name']
                enum_action_strbuf += '\t' + action_name
                if action_len != 1:
                    enum_action_strbuf += ',\n'
                action_len -= 1
        ...
        sm_c_strbuf   = sm_c_header.format(
                            name  = sm_name,
                            onoff = sm_action_onoff
                        )
        ...
        if sm_action_onoff == 1:
            sm_c_strbuf += sm_c_action_enum.format(
                name        = sm_name,
                actions     = enum_action_strbuf
                )
