...
#ifndef BASEWORK_RING_RTE_RINGBUF_H_
#define BASEWORK_RING_RTE_RINGBUF_H_
 ...
 */
static inline void rte_ringbuf_internal_reset(struct ring_buf *buf, int32_t value)
{
 ...
 */
#define RTE_RINGBUF_DECLARE(name, size8, attr) \
	STATIC_ASSERT(size8 < RING_BUFFER_MAX_SIZE,\
		RING_BUFFER_SIZE_ASSERT_MSG); \
	static uint8_t _ring_buffer_data_##name[size8] attr; \
 ...
 */
#define RTE_RINGBUF_ITEM_DECLARE(name, size32, attr) \
	STATIC_ASSERT((size32) < RING_BUFFER_MAX_SIZE / 4,\
		RING_BUFFER_SIZE_ASSERT_MSG); \
	static uint32_t _ring_buffer_data_##name[size32] attr; \
	struct ring_buf name = { \
 ...
 */
#define RTE_RINGBUF_ITEM_SIZEOF(expr) rte_div_roundup(sizeof(expr), sizeof(uint32_t))
 ...
 */
static inline void rte_ringbuf_init(struct ring_buf *buf,
    uint32_t size, uint8_t *data) {
	RTE_ASSERT(size < RING_BUFFER_MAX_SIZE);
	buf->size = size;
	buf->buffer = data;
 ...
 */
static inline void rte_ringbuf_item_init(struct ring_buf *buf,
     uint32_t size, uint32_t *data) {
	RTE_ASSERT(size < RING_BUFFER_MAX_SIZE / 4);
...
#endif /* BASEWORK_RING_RTE_RINGBUF_H_ */
