...
static void get_loadaddr(struct fw_desc *loader) {
    struct firmware_addr addr = {0};
    const uint8_t *p = (const uint8_t *)&addr;
    uint8_t chksum = 0;
    int err;

    err = disk_device_read(loader->dd, &addr, sizeof(addr), 
        FW_INFO_OFFSET + offsetof(struct firmware_pointer, addr));
    if (err) {
    ...
    pr_info("Get load-address(0x%08x) size(0x%x) chksum(0x%x)\n", 
        addr.fw_offset, addr.fw_size, chksum);
    if (chksum == addr.chksum) {
        pr_info("The firmware first load-address(0x%08x) and size(0x%x)\n", 
            addr.fw_offset, addr.fw_size);
        loader->offset = addr.fw_offset;
...
_default:
    ...
    pr_warn("The firmware default load-address(0x%08x)\n", loader->offset);
}
...
static int bootloader_init(void) {
    ...
    pr_dbg("bootloader loadaddr(0x%x) execaddr(0x%x) infoaddr(0x%x)\n", 
        fw_loader.offset, fw_runner.offset, fw_curinfo.offset);
