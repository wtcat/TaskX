...
static void draw_label(lv_event_t * e)
{
	...
	if (code == LV_EVENT_COVER_CHECK) {
		...
		{
			area.y1 = abs_area.y1 + label->height - (label->src_chars[label->indices[0]].header.h) * (label->count) ;
			for (int i = 0; i < label->count; i++) {
				...
				area.x1 = (abs_area.x1 + abs_area.x2 + 1 - src->header.w) / 2;
				area.x2 = area.x1 + src->header.w - 1;
				lv_draw_img(draw_ctx, &img_dsc, &area, src);

				area.y1 += src->header.h;
			}
		}
		else
		{
